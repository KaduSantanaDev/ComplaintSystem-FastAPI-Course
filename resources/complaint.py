...
@router.delete("/complaints/{complaint_id", dependencies=[Depends(oauth2_scheme)], status_code=204)
async def delete_complaint(complaint_id: int):
    await ComplaintManager.delete_complaint(complaint_id)


@router.put("/complaints/{complaint_it}/approve", dependencies=[Depends(oauth2_scheme)], status_code=204)
async def approve_complaint(complaint_id: int):
    await ComplaintManager.approve(complaint_id)


@router.put("/complaints/{complaint_it}/reject", dependencies=[Depends(oauth2_scheme), Depends(is_approver)], status_code=204)
